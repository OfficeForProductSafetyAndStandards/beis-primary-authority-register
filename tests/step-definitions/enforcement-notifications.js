...
When('I raise a new enforcement against a partnership', function () {
    return shared
        ...
        .assert.containsText('h1','Have you discussed this issue with the Primary Authority?')
        .click('#edit-next')
        .assert.containsText('h1','Contact details for enforcement officer')
        .click('#edit-next')
});
When('I choose a coordinated member to enforce', function () {
    return shared
        .click('input[name="par_data_organisation_id"]:nth-child(1)')
        .assert.containsText('h1','Enforce member')
        .click('#edit-next')
});
When('I choose an existing legal entity to enforce', function () {
    return shared
        .click('input[name="legal_entities_select"]:nth-child(1)')
        .assert.containsText('h1','Enforce legal entity')
        .click('#edit-next')
});
...
When('I enter the details of a proposed enforcement', function () {
    return shared
        ...
        .assert.containsText('h1','Enforcement details')
        .click('#edit-next')
});
When('I add an enforcement action {string}', function (title) {
    return shared
        ...
        .assert.containsText('h1','Add an action to the enforcement notice')
        .click('#edit-next')
});
...
When('I review the enforcement notice', function () {
    return shared
        .assert.containsText('h1','Review the enforcement notice')
        .click('#edit-save')
        .assert.containsText('h1','Enforcement notice sent')
        .click('.button')
});
...
When('I search for an enforcement notice {string} against {string}', function (enforcement_notice, organisation) {
    return shared
        ...
        .assert.containsText('h1', 'Enforcements')
...
When('I approve a single enforcement notice', function () {
    return shared
        .assert.containsText('h1', 'Respond to notice of enforcement action')
...
When('I refer a single enforcement notice to {string}', function (authority) {
    return shared
        ...
        .click('#edit-next')
        .assert.containsText('h1', 'Refer Enforcement Notice')
...
When('I check that EO can see valid enforcement notification {string}', function (string) {
  ...
  .waitForElementVisible('.govuk-button--start', 2000)
