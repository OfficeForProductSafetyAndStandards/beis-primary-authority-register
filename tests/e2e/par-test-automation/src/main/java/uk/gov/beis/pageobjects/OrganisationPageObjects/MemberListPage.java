...
public class MemberListPage extends BasePageObject {
	...
	public void searchForAMember(String memberName) {
        waitForElementToBeClickable(By.id("edit-organisation-name"), 3000);
		memberSearchbar.clear();
		memberSearchbar.sendKeys(memberName);
        waitForElementToBeClickable(By.id("edit-submit-members-list"), 3000);
		applyBtn.click();
        waitForPageLoad();
    }

	public void selectMembersName() {
        waitForElementToBeClickable(By.xpath(memberNameLocator.replace("?", DataStore.getSavedValue(UsableValues.MEMBER_ORGANISATION_NAME))),2000);
...
waitForElementToBeVisible(By.xpath(memberNameLocator.replace("?", DataStore.getSavedValue(UsableValues.MEMBER_ORGANISATION_NAME))), 2000);
	...
	public String getMembershipCeasedDate() {
        waitForElementToBeVisible(By.xpath("//td[@class = 'views-field views-field-date-membership-ceased']/time"), 2000);
	...
	public boolean checkMembersListUploaded() {
        waitForElementToBeVisible(By.xpath(memberNameLocator.replace("?", DataStore.getSavedValue(UsableValues.MEMBER_ORGANISATION_NAME))), 3000);
		return driver.findElement(By.xpath(memberNameLocator.replace("?", DataStore.getSavedValue(UsableValues.MEMBER_ORGANISATION_NAME)))).isDisplayed();
	}
