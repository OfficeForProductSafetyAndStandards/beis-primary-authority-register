...
class ParPartnershipFlowsLegalEntityAddForm extends ParBaseForm {
  ...
  public function getFormId() {
    return 'par_partnership_legal_entity';
  }
...
//    $this->loadDataValue("amount", $this->getTempDataValue("amount"));
  }

  public function formDefaults($id = 1) {
    ...
    return [
      ...
      'registered_number' => [
        ...
        '#states' => [
          'visible' => [
            'select[name="legal_entity[' . $id . '][legal_entity_type]"]' => [
  ...
  public function buildForm(array $form, FormStateInterface $form_state, ParDataPartnership $par_data_partnership = NULL, ParDataLegalEntity $par_data_legal_entity = NULL) {
    ...
    $form['amount'] = [
      '#type' => 'hidden',
      '#default_value' => $this->getDefaultValues('amount'),
    ];
    ...
    var_dump($this->getDefaultValues('amount'));

    $loop = 1;

    for ($i = $loop; $i <= $this->getDefaultValues('amount', 1); $i++) {
      $form['legal_entity'][$i] = $this->formDefaults($i);
    }
